...
async def login_to_eitaa(logger, phone_number):
    """
    Launches a browser, navigates to Eitaa Web, enters the phone number,
    and waits for the user to complete the login process manually.
    Returns the browser and page objects on successful login.
    """
    ...
    try:
        ...
        logger.log(f"شماره تلفن '{phone_number}' به صورت خودکار وارد شد.")

        logger.log("🔑 لطفاً کد تایید ارسال شده را در پنجره مرورگر وارد کنید.")
        logger.log("✅ منتظر بمانید تا صفحه اصلی چت‌ها به طور کامل بارگذاری شود...")

        chat_list_container_selector = '#chatlist-container'
        await page.wait_for_selector(chat_list_container_selector, state='visible', timeout=120000) # Increased timeout
        logger.log("✅ ورود با موفقیت تأیید شد. اکنون می‌توانید ربات را اجرا کنید.")

        return browser, page

    except PlaywrightTimeoutError as pte:
        logger.log(f"❌ خطای تایم‌اوت در هنگام ورود: {pte}")
...
async def close_browser(browser, logger):
    """Closes the Playwright browser instance."""
    if browser and browser.is_connected():
        await browser.close()
        logger.log("مرورگر با موفقیت بسته شد.")
    else:
        logger.log("مرورگر قبلاً بسته شده یا متصل نبوده است.")

async def run_tahvil_bot_async(page, config, logger, status_updater):
    ...
    try:
        # Step 1: Search for the group and enter it
        try:
        ...
        # Step 2: Find the target message in the group
        ...
        # Step 3: Extract usernames and send DMs
        logger.log("\n--- شروع مرحله ۳: استخراج و ارسال پیام خصوصی ---")
        ...
        logger.log("\n********************************************************************")
        logger.log("⏹️ تمام عملیات ربات تحویل به پایان رسید.")
...
async def run_id_sender_bot_async(page, config, logger, status_updater):
    ...
    try:
        # Step 1: Read usernames from Excel
        ...
        logger.log("⏹️ عملیات ربات ارسال پیام مستقیم پایان یافت.")
